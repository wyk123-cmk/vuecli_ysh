...
<template>
	<div>
		...
		<div class="content" style="padding-bottom:80px;">
			...
			<ul class="nrwk">
				...
				<li @click="btnCompanyClick">
					...
					<em class="iconfont yjt" style="font-weight:bold;display:flex;overflow:hidden;color:#999999;" v-else>{{ selectCompanyName }}</em>
					物流公司
				</li>
				<li @click="btnSendTime">
            ...
            : ""
              ...
              query: { redirect: this.backUrl,pageType: this.pageType }
				...
				<li @click="btnBj">
					...
					<em class="iconfont yjt" style="font-weight:bold;display:block;overflow:hidden;color:#333;" v-else>
						￥{{ insuranceAll }}
						<i v-if="$util.isNotEmpty(insurance)" style="background:red;color:#fff;font-size:10px;padding:2px 5px;border-radius:7px;">￥{{ insurance }}</i>
					</em>
					保价
		...
		<div class="gjljxdwk">
			<div class="yhq-layer" v-if="isShowYhq">你有{{count}}张可用优惠券，支付时将会自动抵扣<button class="iconfont quxiao" @click="isShowYhq=false"></button></div>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.serviceNames=this.$store.state.serviceNames==""?'提货方式、签单返还等':this.$store.state.serviceNames.toString().replace(/,/g,'-');
		},
		created() {
		...
		methods: {
			...
			GetAddServices() {
				...
				GetServiceList(parms).then(data => {
					...
					this.selList=[{serviceKey:"DeliveryType",serviceValue:this.thid,remark:this.thremark},
			{serviceKey:"BackSignBill",serviceValue:this.qdid,remark:this.qdremark},
			{serviceKey:"PayType",serviceValue:this.fkid,remark:this.fkremark}];
			...
			btnBack() {
				...
				this.$store.state.jkdExpressModelId = "";
				this.$store.state.jkdExpressModelName = "";
				this.$store.state.selectPayType = 1;
			...
			btnGetPrice() {
				if (
					...
					var params = {
						...
						volume: parseFloat(this.goodInfoData.cubic) <= 0 ?
							0 : parseFloat(this.goodInfoData.cubic)
					};
