...
.qydaily li p{line-height: 21px;padding-bottom: 10px;border-bottom: 1px solid #eee;width: calc(100% - 54px);}
.qydaily li p span {
  color: #4285f4;
}
.qydaily li p label {
...
<template>
  <div>
    ...
    <div class="content" ref="scrollBox">
      ...
      <ul class="qydaily" v-infinite-scroll="loadMore"
        ...
        <li v-for="(item,index) in RecordList" :key="index" style="display:flex;">
          ...
          <p>
            <span>{{item.operator }}</span>{{item.logText}} <br />
...
<script>
import {GetCompanyLogList} from '../../api/api'
export default {
  data() {
    return {
      redirect: "",
  ...
  mounted() {
    ...
    GetRecordList(page) {
      ...
      GetCompanyLogList(params).then(data => {
        this.isLoading = false;
        if (data == undefined) {
          this.noneData = true;
          return false;
