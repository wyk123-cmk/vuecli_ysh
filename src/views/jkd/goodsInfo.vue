...
<template>
	<div>
		<header>
			<a class="iconfont icon-back" @click="!this.redirect?$router.go(-1):$router.replace(this.redirect)"></a>
		...
		<div class="content" style="z-index:1">
			<div class="bzlx-bag">
				...
				<input type="search" placeholder="输入物品名称" autocomplete="off" class="packname" v-model="txtGoodName" />
			</div>
			<ul class="nrwk hpxx">
			...
			<ul class="nrwk hpxx">
				<li @click="IsShowPackMaterial = true">
					<span class="iconfont yjt" v-if="txtGoodMaterial==''">{{txtMaterial}}</span>
					<span class="iconfont yjt" v-else>{{ txtGoodMaterial }}</span>包装材质
				</li>
			</ul>
			<ul class="bzlx">
				<h2>寄件备注</h2>
			...
			<ul class="nrwk2">
				<li>
					...
					<span class="yfgs-num">
						...
						<input type="number" v-model="txtnum" style="width:62px;" onkeyup="this.value=this.value.replace(/D/g,'')"
				...
				<li>
					...
					<span class="yfgs-num">
						...
						<input type="number" v-model="txtweight" onkeyup="this.value=this.value.replace(/D/g,'')" onafterpaste="this.value=this.value.replace(/D/g,'')" />
						<i>kg</i>
			...
			<p style="padding-left: 14px; padding-right: 14px; padding-top: 12px; margin-bottom:90px; margin-top: 12px;">
				<input type="button" class="dlnrwk-an" value="确定" @click="btnSumit" />
			</p>
		</div>
		<div class="search-name" v-show="isSearch">
			<ul>
				<li v-for="(item, index) in goodList" :key="index" @click="btnGetGoodName(item.goodsName)">{{ item.goodsName }}</li>
			</ul>
		</div>
		...
		<mt-popup v-model="IsShowPackMaterial" position="bottom" class="mint-popup" style="width:100%;">
			<mt-picker :slots="PackMaterialDatalList" @change="OnPackMaterialChange"></mt-picker>
		</mt-popup>
	</div>
</template>
<script>
	...
	export default {
		data() {
			return {
				...
				PackTypeDatalList: [],
				PackMaterialDatalList: [{
					flex: 1,
					values: ['纸', '纤', '木箱', '木架', '托膜', '托木'],
					className: 'slot1',
					textAlign: 'center'
				}],
				...
				if (this.$util.isNotEmpty(val)) {
					if (this.goodList.length > 0) {
						...
						if (this.$util.isNotEmpty(this.$store.state.goodInfo)) {
							if (this.$store.state.goodInfo.goodName == val) {} else {
								this.getGoodList();
							}
						} else {
							this.getGoodList();
...
document.body.scrollTop = 0
		...
		mounted() {
			...
			// 		console.log(6);
			// 		// this.isSearch=false;
			// 		this.$store.state.goodscg = true;
			// 		this.txtGoodName = this.$store.state.goodInfo.name;
			// 		this.txtweight = this.$store.state.goodInfo.weight;
			// 		this.txtCubic = this.$store.state.goodInfo.volume;
			...
			// 		this.txttalk = this.$store.state.remark;
			// 		this.txtnum = this.$store.state.goodInfo.count;
			// 		this.txtGoodType = this.$store.state.goodInfo.category;
			// 		this.goodMemo = this.$store.state.memo;
			// 	}
			// }
			if ( this.$store.state.goodInfo != null) {
				console.log(65)
				...
				this.txtGoodMaterial=this.$store.state.goodInfo.packageservice
			}
			console.log(this.txtGoodMaterial);
			if (this.pageType == "small") {
		...
		methods: {
			...
			OnPackMaterialChange(picker,values){
				if (values.length <= 0) return;
				this.txtGoodMaterial = values[0];
			},
			btnGetGoodName(val) {
			...
			btnSumit() {
				if (this.Valid()) {
					...
					this.$store.state.goodInfo = {
						...
						packageservice:this.txtGoodMaterial,
