...
<template>
  <div>
    <header>
      ...
      <h1 class="tab-title">资金明细</h1>
    </header>
    <div class="content" ref="scrollBox">
      <div class="xz-date" @click="IsKeyword = true">
        <button>{{ keyWordTime }}<img src="../../assets/images/down.svg" /></button>
      </div>
      <ul class="qymx"  v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
        <li v-for="(item,index) in LogList" :key="index" style="display:flex;">
          <p><span>{{item.operator }}</span>{{item.logText }}<br /><label>{{  $util.formatDate.format(
                  item.createTime,
                  "MM-dd hh:mm"
                )}}</label></p>
          <h2 :style="item.amountSymbol.indexOf('+')>-1?'color:#fd9523':''">{{item.amountSymbol}}</h2>
		  <!-- v-if="item.logType==1" -->
         <!--  <h2 v-if="(item.logType==2 || item.logType==3)" class="zs">+{{item.amount .toFixed(2)}}</h2> -->
        </li>
...
<script>
import { GetCompanyMoneyDetailList } from "../../api/api";
export default {
  data() {
    return {
      redirect: "",
  ...
  methods:{
    ...
    GetRecordList(page) {
      ...
      GetCompanyMoneyDetailList(params).then(data => {
        this.isLoading = false;
        if (data == undefined) {
          this.noneData = true;
          return false;
