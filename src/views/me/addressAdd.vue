...
export default {
  ...
  methods: {
    ...
    btnCopyOk() {
      ...
      // let addressArray = this.$util.AddressAuto(addText, this.addressData);
      var parm = {
        addressText: addText
      };
    ...
    onAddressChange(picker, values) {
      ...
      picker.setSlotValues(1, values[0].list); //values[0].citys);
    ...
    Valid() {
      ...
      if (!this.$util.isPhoneNumber(this.userPhone)) {
    ...
    btnSubmit() {
      ...
      addAddress(addParams).then(data => {
        if (!data) return;
        //创建成功
        MessageBox.alert("创建成功").then(action => {
          var addressData = {
            addressCode: data.contactsCode.trim(),
            userName: data.name.trim(),
            userPhone: data.phone.trim(),
            address: data.address.trim(),
            areaCode: data.areaInfo.areaCode.trim(),
            prociarName:
