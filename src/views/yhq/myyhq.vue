<style scoped>
	...
	.ljsy{position:absolute;   
	    color: #FF4944;
	    background-color: #FFFFFF;
	    padding: 4px 10px;
	    border-radius: 16px;
	    display: block;top:31px;right: 12px;}
</style>
<template>
	<div>
		<header>
		...
		<ul class="ljzxwk">
			...
			<li class="hsbj" v-for="(item,i) in listData" :key="i">
				...
				<span class="jeishao-sh"><s v-if="item.status=='2' || item.status=='3'">{{item.orderNumDetail}}</s><br><s
					 style="font-size:14px;">{{item.fullReducAmount}}</s><br>{{item.overdate}}</span>	<span class="ljsy" v-if="item.claimType =='0'" @click="Express(item.amount)">立即使用</span>
